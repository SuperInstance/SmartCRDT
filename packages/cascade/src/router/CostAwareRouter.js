...
export class CostAwareRouter {
    ...
    DEFAULT_LOCAL_MODEL = "llama2:70b";
    DEFAULT_CLOUD_MODEL = "gpt-3.5-turbo";
    DEFAULT_PERFORMANCE_MODEL = "gpt-4-turbo";
    constructor(config = { mode: "balanced" }) {
        this.config = {
            mode: config.mode,
     ...
     */
    route(query, complexity = 0.5, context) {
        // Estimate tokens
        const tokenEstimate = this.tokenEstimator.estimate(query);
        const estimatedOutputTokens = this.tokenEstimator.estimateOutputFromInput(tokenEstimate.inputTokens, complexity);
        // Get routing recommendation based on mode
        const recommendation = this.getRecommendation(query, complexity, tokenEstimate.inputTokens, estimatedOutputTokens);
        // Check budget
        ...
        // Add notes
        result.notes?.push(`Tokens: ~${tokenEstimate.inputTokens} input, ~${estimatedOutputTokens} output`);
        ...
        if (recommendation.estimatedCost > this.config.maxCostPerRequest) {
            result.notes?.push(`WARNING: Cost exceeds max per-request limit ($${this.config.maxCostPerRequest.toFixed(4)})`);
        }
        // Record cost if tracking budget
        if (this.budgetTracker && withinBudget) {
            this.budgetTracker.recordCost(recommendation.model, recommendation.estimatedCost);
        }
        return result;
    }
    /**
     ...
     */
    getRecommendation(query, complexity, inputTokens, outputTokens) {
        switch (this.config.mode) {
            case "economy":
                return this.getEconomyRecommendation(query, complexity, inputTokens, outputTokens);
            case "performance":
                return this.getPerformanceRecommendation(query, complexity, inputTokens, outputTokens);
            case "balanced":
            default:
                return this.getBalancedRecommendation(query, complexity, inputTokens, outputTokens);
        }
    }
    /**
     * Economy mode recommendation - minimize cost
     */
    getEconomyRecommendation(query, complexity, inputTokens, outputTokens) {
        // Always try local first
        const localModel = this.pricing.getCheapestModel(Math.ceil(complexity * 3), true);
        if (localModel) {
            const pricing = this.pricing.getModelPricing(localModel);
            const cost = this.pricing.estimateCost(localModel, inputTokens, outputTokens);
        ...
        const cheapestCloud = this.pricing.getCheapestModel(3, false) || this.DEFAULT_CLOUD_MODEL;
        const pricing = this.pricing.getModelPricing(cheapestCloud);
        const cost = this.pricing.estimateCost(cheapestCloud, inputTokens, outputTokens);
        return {
            model: cheapestCloud,
     ...
     */
    getPerformanceRecommendation(query, complexity, inputTokens, outputTokens) {
        // Use best quality cloud model
        const bestModel = this.pricing.getBestQualityModel() || this.DEFAULT_PERFORMANCE_MODEL;
        const pricing = this.pricing.getModelPricing(bestModel);
        const cost = this.pricing.estimateCost(bestModel, inputTokens, outputTokens);
        return {
            model: bestModel,
            provider: pricing.provider,
            qualityTier: pricing.qualityTier,
     ...
     */
    getBalancedRecommendation(query, complexity, inputTokens, outputTokens) {
        // Weights for scoring
        ...
        for (const model of allModels) {
            ...
            if (pricing.qualityTier < requiredTier)
                continue;
        ...
        if (!best) {
            ...
            const fallbackCost = this.pricing.estimateCost(this.DEFAULT_CLOUD_MODEL, inputTokens, outputTokens);
            return {
     ...
     */
    estimateCost(query, model, outputTokens) {
        const tokenEstimate = this.tokenEstimator.estimate(query);
        const estimatedOutput = outputTokens ??
            this.tokenEstimator.estimateOutputFromInput(tokenEstimate.inputTokens);
        const cost = this.pricing.estimateCost(model, tokenEstimate.inputTokens, estimatedOutput);
 ...
 */
export function createCostAwareRouter(config) {
...
//# sourceMappingURL=CostAwareRouter.js.map