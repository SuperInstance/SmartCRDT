...
export class MetricsServer {
  ...
  private wsClients: Set<WebSocketConnection> = new Set();
  ...
  constructor(
    ...
    this.config = {
      ...(collector.getSnapshot() as any), // Get existing config
  ...
  async start(): Promise<void> {
    ...
    console.log(
      `Dashboard available at http://${host}:${port}${this.config.dashboard?.path || "/metrics"}`
    );
  }
   ...
   */
  private async serveRequestMetrics(
    req: IncomingMessage,
    res: ServerResponse
  ): Promise<void> {
    const snapshot = this.collector.getSnapshot();
   ...
   */
  private async serveLatencyMetrics(
    req: IncomingMessage,
    res: ServerResponse
  ): Promise<void> {
    const snapshot = this.collector.getSnapshot();
   ...
   * Serve cache metrics
   */
  private async serveCacheMetrics(
    req: IncomingMessage,
    res: ServerResponse
  ): Promise<void> {
    const snapshot = this.collector.getSnapshot();
   ...
   * Serve cost metrics
   */
  private async serveCostMetrics(
    req: IncomingMessage,
    res: ServerResponse
  ): Promise<void> {
    const snapshot = this.collector.getSnapshot();
   ...
   */
  private async serveHealthMetrics(
    req: IncomingMessage,
    res: ServerResponse
  ): Promise<void> {
    const snapshot = this.collector.getSnapshot();
  ...
  private async serveQuery(
    ...
    const end = parseInt(
      url.searchParams.get("end") || Date.now().toString(),
      10
    );
    const window = url.searchParams.get("window") as AggregationWindow;
    const aggregation = url.searchParams.get("aggregation") as
      | "sum"
      | "avg"
      | "min"
      | "max"
   ...
   */
  private sendToClient(client: WebSocketConnection, message: any): void {
