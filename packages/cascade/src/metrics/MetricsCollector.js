...
export class MetricsCollector {
    ...
    getRequestMetrics() {
        ...
        for (const metric of requests) {
            if (metric.labels?.backend) {
                byBackend[metric.labels.backend] +=
                    metric.value;
            }
    ...
    getCacheMetrics() {
        ...
        for (const metric of cacheOps) {
            const cacheType = metric.labels?.cacheType;
        ...
        for (const type of Object.keys(byType)) {
            const typeTotal = byType[type].hits +
                byType[type].misses;
            byType[type].hitRate =
                typeTotal > 0
                    ? byType[type].hits / typeTotal
                    : 0;
        }
        ...
        const avgEntrySize = sizeMetrics.length > 0 ? this.average(sizeMetrics.map(m => m.value)) : 0;
        return {
            hits,
            misses,
    ...
    getCostMetrics() {
        ...
        const estimatedMonthly = uptimeHours > 0 ? (total / uptimeHours) * 24 * 30 : 0;
        return {
            total,
            byBackend,
            byModel,
    ...
    getHealthMetrics() {
        ...
        for (const metric of healthMetrics) {
            const backendName = metric.labels?.backend;
            if (backendName) {
     ...
     */
    getLatencyDistribution(latencies) {
     ...
     */
    getTimeSeries(name, window = 60000) {
