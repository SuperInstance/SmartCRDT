...
export class QueryRefiner {
  ...
  constructor(private config?: QueryRefinerConfig) {
   ...
   */
  async refine(query: string): Promise<RefinedQuery> {
    ...
    const semanticFeatures =
      this.config?.enableSemantic !== false
   ...
   */
  private analyzeStatic(query: string): StaticFeatures {
  ...
  private async analyzeSemantic(
    ...
    if (cachedEmbedding) {
      ...
      if (result.usedFallback) {
        console.warn(
          `[QueryRefiner] Using fallback embeddings for query: "${query.slice(0, 50)}..."`
        );
      }
    }
   ...
   */
  private detectQueryType(query: string, lowerQuery: string): QueryType {
    // Question patterns
    if (
      /^(what|how|why|when|where|who|which|whose|can|could|would|should|is|are|do|does|did)\b/i.test(
        query
      )
    ) {
      return "question";
    ...
    // Command patterns
    if (
      /^(create|make|build|write|generate|implement|add|remove|delete|update)\b/i.test(
        query
      )
    ) {
      return "command";
    ...
    // Comparison patterns
    if (
      /\b(compare|difference|versus|vs|better than|worse than)\b/i.test(query)
    ) {
      return "comparison";
    }
    ...
    // Debug patterns
    if (
      /\b(debug|fix|error|bug|issue|problem|broken|not working)\b/i.test(query)
    ) {
      return "debug";
    }
   ...
   */
  private calculateComplexity(query: string, lowerQuery: string): number {
    let complexity = 0;
   ...
   */
  private hasSQL(query: string): boolean {
    const sqlKeywords =
      /\b(SELECT|INSERT|UPDATE|DELETE|FROM|WHERE|JOIN|GROUP BY|ORDER BY|CREATE TABLE|ALTER TABLE)\b/i;
   ...
   */
  private hasUrl(query: string): boolean {
   ...
   */
  private hasEmail(query: string): boolean {
   ...
   */
  private calculateCapitalizationRatio(query: string): number {
   ...
   */
  private calculatePunctuationDensity(query: string): number {
   ...
   */
  private extractTechnicalTerms(lowerQuery: string): string[] {
   ...
   */
  private extractDomainKeywords(lowerQuery: string): string[] {
   ...
   */
  private detectCluster(query: string, embedding: number[]): string | null {
   ...
   */
  private calculateSemanticComplexity(embedding: number[]): number {
  ...
  private generateSuggestions(
    ...
    if (
      semanticFeatures?.similarQueries &&
   ...
   */
  private trackQuery(query: string, timestamp: number): void {
    // Track frequency
 ...
 */
export const DEFAULT_REFINER_CONFIG: QueryRefinerConfig = {
  enableSemantic: true,
