...
export class TokenEstimator {
     ...
     */
    estimateConversation(input, output) {
    ...
    estimateMessages(messages) {
        ...
        return {
            ...
            method: methods.size === 1 ? methods.values().next().value : "mixed",
        };
    }
    /**
     ...
     */
    estimateByLanguage(text, detection) {
        const trimmed = text.trim();
     ...
     */
    estimateOutputFromInput(inputTokens, queryComplexity = 0.5) {
     ...
     */
    estimateWithSystemPrompt(userMessage, systemPrompt, conversationHistory = []) {
        // Count system prompt
        const systemTokens = this.estimate(systemPrompt).inputTokens;
        // Count conversation history
        const historyTokens = this.estimateMessages(conversationHistory).inputTokens;
 ...
 */
export function estimateConversationTokens(input, output) {
