...
export class CacheWarmer {
  ...
  async warm(): Promise<CacheWarmingResult> {
        ...
        batch.map(async query => {
          try {
            ...
            if (!decision.notes?.includes("Cache hit")) {
              await this.router.cacheResult?.(
                (await this.router
                  .getSessionContext()
                  ?.getRecentQueries(1)[0]) as any,
                decision
              );
            }
    ...
    console.log(
      `[CacheWarmer] Warming complete: ${successful} successful, ${failed} failed, ${duration}ms`
    );
   ...
   */
  async warmInBackground(): Promise<CacheWarmingResult> {
 ...
 */
export const DEFAULT_CACHE_WARMER_CONFIG: Partial<CacheWarmerConfig> = {
  batchSize: 10,
  delayBetweenBatches: 100,
};
