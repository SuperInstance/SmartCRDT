...
async function generateDataset() {
    ...
    const outputDir = args[1] || path.join(process.cwd(), "packages/cascade/data");
    ...
    console.log(`✓ Generated ${queries.length} queries in ${duration}ms (${(duration / count).toFixed(2)}ms per query)`);
    // Initialize exporter
    const exporter = new DataExporter();
    // Add writeFile method to exporter for Node.js
    exporter.writeFile = async (filePath, content) => {
    ...
    await fs.writeFile(path.join(outputDir, "statistics.json"), JSON.stringify(stats, null, 2), "utf-8");
    ...
    for (const [intent, count] of Object.entries(stats.intents)) {
        ...
        console.log(`  ${intent.padEnd(20)} ${countNum.toString().padStart(5)} (${percentage}%)`);
    }
    console.log();
