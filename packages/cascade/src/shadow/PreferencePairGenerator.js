...
export class PreferencePairGenerator {
    ...
    generateFromLogs(logs) {
        ...
        for (const [normalizedQuery, entries] of queryEntries) {
            ...
            for (let i = 1; i <= numPairs; i++) {
                ...
                if (chosen.score.total - rejected.score.total <
                ...
                pairs.push({
                    ...
                    chosenMetadata: {
                        ...
                        cost: chosen.entry.metadata?.cost ?? 0,
                        latency: chosen.entry.metadata?.latency ?? 0,
                    ...
                    rejectedMetadata: {
                        ...
                        cost: rejected.entry.metadata?.cost ?? 0,
                        latency: rejected.entry.metadata?.latency ?? 0,
    ...
    calculateScore(entry) {
        ...
        const latency = entry.metadata?.latency ?? 0;
        ...
        const cost = entry.metadata?.cost ?? 0;
        ...
        const cacheBonus = entry.metadata?.fromCache
            ...
            : 0;
        // Total score
        const total = quality * this.config.qualityWeight -
            costPenalty -
            latencyPenalty +
            cacheBonus;
        return {
     ...
     */
    generateReason(chosenScore, rejectedScore) {
        const reasons = [];
        // Quality difference
    ...
    calculateStats(pairs) {
        ...
        const totalChosenQuality = pairs.reduce((sum, p) => sum + p.chosenMetadata.quality, 0);
        const totalRejectedQuality = pairs.reduce((sum, p) => sum + p.rejectedMetadata.quality, 0);
        const totalScoreDiff = pairs.reduce((sum, p) => sum + (p.chosenMetadata.score - p.rejectedMetadata.score), 0);
        const localChosen = pairs.filter(p => p.chosenMetadata.backend === "local").length;
        const cloudChosen = pairs.filter(p => p.chosenMetadata.backend === "cloud").length;
        return {
            total: pairs.length,
     ...
     */
    filterByQuality(pairs, minQuality) {
     ...
     * @returns Filtered preference pairs
     */
    filterByCostDifference(pairs, minDiff) {
        return pairs.filter(pair => Math.abs(pair.chosenMetadata.cost - pair.rejectedMetadata.cost) >=
            minDiff);
    }
    /**
    ...
    balanceByBackend(pairs) {
        ...
        // Shuffle and combine
        return [...balancedLocalPairs, ...balancedCloudPairs].sort(() => Math.random() - 0.5);
    }
}
/**
 ...
 * @returns Array of preference pairs
 */
export function generatePreferencePairs(logs) {
    const generator = new PreferencePairGenerator();
 ...
 * @returns JSONL-formatted string
 */
export function exportForORPO(pairs) {
    const generator = new PreferencePairGenerator();
    return generator.exportForORPO(pairs);
}
//# sourceMappingURL=PreferencePairGenerator.js.map