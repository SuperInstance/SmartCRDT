...
export class DataExporter {
  ...
  private stratifiedSplit(
    ...
    const byIntent = new Map<string, GeneratedQuery[]>();
    ...
    const train: GeneratedQuery[] = [];
    const validation: GeneratedQuery[] = [];
    const test: GeneratedQuery[] = [];
   ...
   */
  private checkBalance(counts: Map<string, number>): "balanced" | "imbalanced" {
  ...
  generateStatsReport(stats: DatasetStatistics): string {
    ...
    lines.push(
      `**Query Length Range:** ${stats.minQueryLength} - ${stats.maxQueryLength} chars\n`
    );
