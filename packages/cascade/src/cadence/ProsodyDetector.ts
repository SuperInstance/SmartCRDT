...
export class ProsodyDetector {
  private history: ProsodyFeaturesEntry[] = [];
   ...
   */
  detect(text: string, timestamp: number = Date.now()): ProsodyFeatures {
    const now = timestamp;
  ...
  getTrend(): TrendAnalysis {
    ...
    // Calculate averages
    const avgWpm =
      recent.reduce((sum, entry) => sum + entry.features.wordsPerMinute, 0) /
      recent.length;
    ...
    let wpmTrend: "increasing" | "stable" | "decreasing";
  ...
  isUrgent(): boolean {
    ...
    return (
      trend.wpmTrend === "increasing" && latest.features.wpmAcceleration > 20
    );
  }
  ...
  isInFlow(): boolean {
    const trend = this.getTrend();
    return (
      trend.avgSilence < this.config.continuousThresholdMs &&
...
/**
 * Internal entry for storing history with timestamp
 */
interface ProsodyFeaturesEntry {
