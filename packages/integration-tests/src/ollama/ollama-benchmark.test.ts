 ...
 *
 * Comprehensive performance benchmarking for Ollama inference.
 * Tests latency, throughput, memory usage, and concurrent request handling.
 *
 * Run with: npx vitest run ollama-benchmark.test.ts --reporter=verbose
 */
...
const BENCHMARK_CONFIG = {
  ...
  THROUGHPUT_DURATION: 10000, // 10 seconds
...
beforeAll(async () => {
  try {
    await axios.get(`${BENCHMARK_CONFIG.OLLAMA_BASE_URL}/api/tags`, {
      timeout: 2000,
    });
    isOllamaAvailable = true;
    console.log(`✅ Ollama available at ${BENCHMARK_CONFIG.OLLAMA_BASE_URL}`);
  } catch (error) {
    console.warn("⚠️  Ollama not available. Running benchmarks with mocks.");
  }
});
...
describe("Ollama Performance Benchmarks", () => {
  /**
   * Latency benchmarks
   */
  ...
  describe("Throughput Benchmarks", () => {
    it("benchmark: sustained throughput (10 seconds)", async () => {
      const adapter = isOllamaAvailable
        ? new OllamaAdapter(
            BENCHMARK_CONFIG.OLLAMA_BASE_URL,
            BENCHMARK_CONFIG.OLLAMA_MODEL,
            { timeout: 30000, maxRetries: 3, stream: false }
          )
        : createMockOllamaAdapter(100, 0);
  ...
  describe("Concurrency Benchmarks", () => {
    it.concurrent("benchmark: concurrency levels", async () => {
      ...
      // (up to a point, then diminishing returns)
  ...
  describe("Comparison Benchmarks", () => {
    it("benchmark: compare different query types", async () => {
      ...
      const queryTypes = [
        { name: "Simple", query: "What is 2+2?" },
        { name: "Factual", query: "What is the capital of France?" },
        { name: "Complex", query: "Explain quantum computing" },
      ];
...
afterAll(() => {
  console.log("\n" + "=".repeat(60));
  console.log("OLLAMA PERFORMANCE BENCHMARK SUMMARY");
  console.log("=".repeat(60));
  console.log(`Ollama Available: ${isOllamaAvailable ? "✅ Yes" : "❌ No (using mocks)"}`);
  console.log(`Model: ${BENCHMARK_CONFIG.OLLAMA_MODEL}`);
  console.log("\nThresholds:");
