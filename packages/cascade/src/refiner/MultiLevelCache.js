...
export class MultiLevelCache {
    ...
    l3Enabled;
    l2Cache = new Map();
    l2AccessOrder = []; // For LRU tracking
    l3CacheDir;
    writeThrough;
    promoteOnHit;
    // Temperature tracking
    temperatureThreshold = 0.7; // Entries above this are "hot"
    coldThreshold = 0.3; // Entries below this are "cold"
    // Statistics
    stats = {
     ...
     */
    calculateL2EvictionScore(key, entry) {
        const position = this.l2AccessOrder.indexOf(key);
        const lruScore = position >= 0
    ...
    // ========== L3 (Disk) Methods ==========
    async getL3(cacheKey) {
        try {
            const filePath = join(this.l3CacheDir, `${this.sanitizeKey(cacheKey)}.json`);
 ...
 */
export const DEFAULT_MULTI_LEVEL_CACHE_CONFIG = {
    l1: {
        maxSize: 1000,
...
//# sourceMappingURL=MultiLevelCache.js.map