...
export class CacheOptimizer {
    ...
    generateReport() {
        ...
        for (const [type, stats] of Object.entries(metrics.perQueryTypeStats)) {
            if (stats.count > 0) {
                lines.push(`  ${type.padEnd(12)} ${(stats.hitRate * 100).toFixed(1)}% hit rate, ` +
     ...
     */
    async applyThresholdOptimization(suggestion) {
        ...
        if (action.includes("decrease") || action.includes("lower")) {
            ...
            newThreshold = Math.max(0.7, this.cache.getSimilarityThreshold() - decreaseBy);
        }
        else if (action.includes("increase") || action.includes("raise")) {
            const increaseBy = 0.02;
            newThreshold = Math.min(0.95, this.cache.getSimilarityThreshold() + increaseBy);
        }
     ...
     */
    async applySizeOptimization(suggestion) {
        const action = suggestion.action.toLowerCase();
        let newSize;
     ...
     */
    async applyTTLOptimization(suggestion) {
     ...
     */
    async applyWarmingOptimization(suggestion) {
     ...
     */
    async applyMemoryOptimization(suggestion) {
        const action = suggestion.action.toLowerCase();
 ...
 */
export function createOptimizer(cache) {
 ...
 */
export function needsOptimization(cache) {
    const optimizer = new CacheOptimizer(cache);
    const suggestions = optimizer.analyze();
    // Need optimization if there are high or medium priority suggestions
    return suggestions.some(s => s.priority === "high" || s.priority === "medium");
}
/**
 * Auto-optimization
 *
 ...
 */
export async function autoOptimize(cache) {
    const optimizer = new CacheOptimizer(cache);
    const suggestions = optimizer.analyze();
    const results = [];
