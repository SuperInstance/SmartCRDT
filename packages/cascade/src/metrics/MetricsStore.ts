...
export class MetricsStore {
  ...
  constructor(config: MetricsConfig) {
    ...
    this.maxBuckets = Math.ceil(
      (config.retentionHours * 3600000) / this.bucketSize
    );
  }
  ...
  write(metric: Metric): void {
    const bucketTimestamp =
      Math.floor(metric.timestamp / this.bucketSize) * this.bucketSize;
...
export class RedisMetricsStore {
  private config: MetricsConfig;
  private redis: any; // Redis client (lazy loaded)
  ...
  async query(options: {
    ...
    for (const key of keys) {
      ...
      for (const value of values) {
        const metric: Metric = JSON.parse(value);
        if (metric.timestamp >= start && metric.timestamp <= end) {
          results.push(metric);
        }
      }
   ...
   */
  private getRedisKey(metric: Metric): string {
