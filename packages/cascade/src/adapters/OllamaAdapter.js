...
export class OllamaAdapter {
     ...
     */
    constructor(baseURL, defaultModel, config, rateLimiter) {
    ...
    async execute(decision, input) {
        ...
        if (this.rateLimiter) {
            ...
            if (!canProceed) {
                ...
                throw new RateLimitError(`Rate limit exceeded: maximum ${stats.maxRequests} requests per ${stats.windowMs}ms`, waitTime, stats);
            }
        }
        ...
        try {
            ...
            const response = await this.withRetry(async () => {
                const result = await this.axiosInstance.post("/api/generate", request);
                return result.data;
            });
    ...
    async checkHealth() {
        try {
            const result = await this.axiosInstance.get("/api/tags");
    ...
    handleError(error, context) {
        ...
        if (axios.isAxiosError(error)) {
            ...
            if (axiosError.code === "ECONNREFUSED") {
                return new OllamaAdapterError(`Ollama service unreachable at ${this.config.baseURL}. Is Ollama running?`, "ECONNREFUSED", error);
            }
            if (axiosError.code === "ETIMEDOUT" ||
                axiosError.code === "ECONNABORTED") {
                return new OllamaAdapterError(`Request timeout after ${this.config.timeout}ms`, "TIMEOUT", error);
            }
            if (axiosError.response?.status === 404) {
                return new OllamaAdapterError(`Model not found: ${this.config.defaultModel}`, "MODEL_NOT_FOUND", error);
            }
            if (axiosError.response?.status === 500) {
                return new OllamaAdapterError("Ollama internal server error", "INTERNAL_ERROR", error);
            }
            return new OllamaAdapterError(`HTTP ${axiosError.response?.status}: ${axiosError.message}`, "HTTP_ERROR", error);
        }
        if (error instanceof Error) {
            return new OllamaAdapterError(`${context}: ${error.message}`, "UNKNOWN_ERROR", error);
        }
        return new OllamaAdapterError(`${context}: Unknown error`, "UNKNOWN_ERROR", error);
    }
    /**
 ...
 * ```
 */
export function createOllamaAdapter(baseURL, defaultModel, config, rateLimiter) {
