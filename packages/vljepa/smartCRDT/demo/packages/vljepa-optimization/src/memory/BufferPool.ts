...
export class BufferPool {
  ...
  resize(targetSize: number): void {
    ...
    if (targetSize > currentSize) {
      ...
      const toRemove = Math.min(this.available.length, currentSize - targetSize);
   ...
   */
  isHealthy(): boolean {
    return this.available.length > 0 || !this.config.maxSize || this.size() < this.config.maxSize;
  }
  ...
  private shouldShrink(): boolean {
    ...
    if (this.config.shrinkStrategy === 'aggressive') {
      ...
      return utilizationRate < 0.1 && idleTime > 30000 && this.available.length > 4;
...
export class MemoryArena {
  ...
  getStats() {
    ...
    const usedSize = Array.from(this.allocations.values()).reduce((sum, a) => sum + a.size, 0);
