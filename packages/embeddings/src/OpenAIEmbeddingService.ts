...
export class OpenAIEmbeddingService {
  ...
  constructor(config: EmbeddingServiceConfig = {}) {
    ...
    this.config = {
      ...
      model,
      dimensions:
        config.dimensions || OpenAIEmbeddingService.MODEL_DIMENSIONS[model],
  ...
  async initialize(): Promise<void> {
    ...
    if (!this.config.apiKey && !this.isOllama()) {
      console.warn(
        "[OpenAIEmbeddingService] No API key provided. Hash-based fallback will be used."
      );
      console.warn(
        "[OpenAIEmbeddingService] WARNING: Hash-based embeddings are NOT semantically meaningful."
      );
      console.warn(
        "[OpenAIEmbeddingService] For production use, provide OPENAI_API_KEY or use Ollama for local embeddings."
      );
    }
  ...
  async embed(text: string): Promise<EmbeddingResult> {
    ...
    try {
      ...
      if (this.config.enableFallback) {
        console.warn(
          "[OpenAIEmbeddingService] API call failed, using hash-based fallback:",
          error
        );
        console.warn(
          "[OpenAIEmbeddingService] WARNING: Fallback embeddings are NOT semantically meaningful."
        );
        console.warn(
          "[OpenAIEmbeddingService] Semantic features (similarity, search, clustering) will NOT work correctly."
        );
  ...
  async embedBatch(texts: string[]): Promise<EmbeddingResult[]> {
    ...
    for (let i = 0; i < texts.length; i += batchSize) {
      ...
      try {
        ...
        if (this.config.enableFallback) {
          console.warn(
            "[OpenAIEmbeddingService] Batch API call failed, using fallback:",
            error
          );
          console.warn(
            "[OpenAIEmbeddingService] WARNING: Fallback embeddings are NOT semantically meaningful."
   ...
   */
  private async embedWithRetry(text: string): Promise<Float32Array> {
   ...
   */
  private async callOpenAIAPI(texts: string[]): Promise<Float32Array[]> {
   ...
   */
  private async callOllamaAPI(texts: string[]): Promise<Float32Array[]> {
    ...
    for (const text of texts) {
      ...
      if (!response.ok) {
        const error = await response.text();
        throw new Error(
          `Ollama API error: ${response.status} ${response.statusText} - ${error}`
        );
      }
