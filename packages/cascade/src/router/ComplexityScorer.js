...
export class ComplexityScorer {
    ...
    score(query, context) {
        ...
        const termCount = this.technicalTerms.filter(term => lowerQuery.includes(term)).length;
        ...
        // Calculate overall
        const overall = Math.min(textLength + technicalTerms + codePresence + domainSpecificity, 1);
        return {
            overall,
            textLength,
            technicalTerms,
