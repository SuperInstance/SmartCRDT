...
async function main(): Promise<void> {
  ...
  console.log(
    `✓ Generator initialized with ${Object.values(templateStats).reduce((a, b) => a + b, 0)} templates`
  );
  ...
  if (options.formats.includes("jsonl")) {
    await exporter.exportToJSONL(
      queries,
      `${options.outputDir}/training.jsonl`
    );
  ...
  const splits = exporter.splitTrainValTest(
    queries,
    0.8,
    0.1,
    0.1,
  ...
  if (options.generateStats) {
    ...
    await exporter.exportToJSON(
      [{ data: stats }] as any,
      `${options.outputDir}/statistics.json`
    );
