...
export class OpenAIAdapterError extends AdapterError {
  ...
  public readonly openaiRecovery?: OpenAIRecoveryInfo;

  /** Request ID from OpenAI (for support) */
  public readonly openaiRequestId?: string;
  ...
  private constructor(
    ...
    this.openaiRecovery = context.openaiRecovery as
      | OpenAIRecoveryInfo
      | undefined;
    this.openaiRequestId = context.openaiRequestId as string | undefined;
  }
  ...
  static rateLimit(
    ...
    return new OpenAIAdapterError(
      "generate",
      `OpenAI rate limit exceeded. Please retry after ${retryAfter || 60} seconds.`,
  ...
  static quotaExceeded(cause?: Error): OpenAIAdapterError {
    return new OpenAIAdapterError(
      "generate",
      "OpenAI quota exceeded. Please check your usage and billing details.",
  ...
  static contentFilter(
    message: string = "Content flagged by OpenAI content filter",
    cause?: Error
  ): OpenAIAdapterError {
    return new OpenAIAdapterError(
      "generate",
  ...
  static contextLengthExceeded(
    ...
    return new OpenAIAdapterError(
      "generate",
      `Context length exceeded for ${modelName}. Maximum: ${maxLength}, Actual: ${actualLength}`,
      OpenAIErrorCode.CONTEXT_LENGTH,
      {
        severity: ErrorSeverity.MEDIUM,
  ...
  static fromAxiosError(
    ...
    if (statusCode === 429) {
      ...
      return new OpenAIAdapterError(
        "generate",
        `OpenAI rate limit exceeded. Please retry after ${rateLimitInfo.retryAfter || 60} seconds.`,
        OpenAIErrorCode.RATE_LIMIT,
        {
          ...baseContext,
          severity: ErrorSeverity.MEDIUM,
    ...
    if (statusCode === 404) {
      return new OpenAIAdapterError(
        "generate",
        `Model not found: ${errorParam || "unknown"}`,
        OpenAIErrorCode.MODEL_NOT_FOUND,
        {
          ...baseContext,
          severity: ErrorSeverity.HIGH,
    ...
    if (statusCode === 400) {
      ...
      if (errorCode === "context_length_exceeded") {
        ...
        return new OpenAIAdapterError(
          "generate",
          `Context length exceeded for model. Maximum: 0, Actual: 0`,
          OpenAIErrorCode.CONTEXT_LENGTH,
          {
            ...baseContext,
            severity: ErrorSeverity.MEDIUM,
